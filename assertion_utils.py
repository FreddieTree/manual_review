...
def update_assertion(original, updated_fields, updater, pmid, sentence_idx, sentence_text, comment=""):
    ...
    # determine if it's modify vs re-add: if anything changed it's modify
    changed = [f for f in fields if original.get(f) != updated_fields.get(f, original.get(f))]
    action_type = "modify" if changed else "accept"
    ...
    updated["related_to"] = original.get("assertion_id")
...
def audit_review_submission(abs_id, sentence_results, post_data, reviewer_info):
    ...
    for sent_idx, sent in enumerate(sentence_results):
        for ass_idx, assertion in enumerate(sent.get("assertions", [])):
            ...
            changed = (
                assertion.get("subject") != post_data.get(subject_key, assertion.get("subject")) or
                assertion.get("predicate") != post_data.get(predicate_key, assertion.get("predicate")) or
                assertion.get("object") != post_data.get(object_key, assertion.get("object")) or
                str(assertion.get("negation")).lower() != str(post_data.get(negation_key, assertion.get("negation"))).lower()
            )
            updated_fields = {
                "subject": post_data.get(subject_key, assertion.get("subject")),
                "subject_type": post_data.get(subject_type_key, assertion.get("subject_type")),
                "predicate": post_data.get(predicate_key, assertion.get("predicate")),
                "object": post_data.get(object_key, assertion.get("object")),
                "object_type": post_data.get(object_type_key, assertion.get("object_type")),
                "negation": post_data.get(negation_key, str(assertion.get("negation"))).lower() == "true",
            }
    ...
    for sent_idx, sent in enumerate(sentence_results):
        ...
        subj_type = post_data.get(f"useradd_subject_type_{sent_idx}", "").strip() or ""
        obj_type = post_data.get(f"useradd_object_type_{sent_idx}", "").strip() or ""
